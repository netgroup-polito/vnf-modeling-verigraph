...
public class Rule_Router extends NetworkObject {
	...
	public void installRouter() {
		...
		constraints.add(ctx.mkForall(new Expr[] { p_0, n_0 },
				ctx.mkImplies((BoolExpr) nctx.send.apply(n_Router, n_0, p_0),
						ctx.mkExists(new Expr[] { p_1, n_1 }, ctx.mkAnd((BoolExpr) nctx.recv.apply(n_1, n_Router, p_1),
								(BoolExpr) matchEntry.apply(nctx.pf.get("dest").apply(p_1)),
