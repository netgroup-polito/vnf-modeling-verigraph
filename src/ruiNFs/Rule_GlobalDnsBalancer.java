...
public class Rule_GlobalDnsBalancer extends NetworkObject {
	...
	protected void init(Context ctx, Object[]... args) {
		...
		matchEntry = ctx.mkFuncDecl(n_GlobalDnsBalancer + "_matchEntry",
				new Sort[] { nctx.address, ctx.mkIntSort(), nctx.address }, ctx.mkBoolSort());
	...
	protected void addConstraints(Solver solver) {
		...
		for (int i = 0; i < entries.size(); i++) {
			entry_map[i] = ctx.mkAnd(ctx.mkEq(e_0, entries.get(i).get(0)), ctx.mkEq(e_1, entries.get(i).get(1)),
					ctx.mkEq(e_2, entries.get(i).get(2)));
		}
		solver.add(ctx.mkForall(new Expr[] { e_0, e_1, e_2 },
				ctx.mkEq(matchEntry.apply(e_0, e_1, e_2), ctx.mkOr(entry_map)), 1, null, null, null, null));
	}
	...
	public void addEntry(Expr expr_0, Expr expr_1, Expr expr_2) {
		if (expr_0 == null && expr_1 == null && expr_2 == null)
			return;
	...
	public void installGlobalDnsBalancer() {
		...
		constraints.add(ctx.mkForall(new Expr[] { p_0, n_0 }, ctx.mkImplies(
				ctx.mkAnd((BoolExpr) nctx.send.apply(n_GlobalDnsBalancer, n_0, p_0),
						ctx.mkEq(nctx.pf.get("proto").apply(p_1), ctx.mkInt(nctx.DNS_REQUEST))),
				ctx.mkExists(new Expr[] { p_1, n_1 },
						ctx.mkAnd((BoolExpr) nctx.recv.apply(n_1, n_GlobalDnsBalancer, p_1),
								ctx.mkEq(nctx.pf.get("proto").apply(p_1), ctx.mkInt(nctx.DNS_REQUEST)),
								(BoolExpr) matchEntry.apply(nctx.pf.get("src").apply(p_1),
