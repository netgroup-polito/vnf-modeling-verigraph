...
public class Rule_VpnExit extends NetworkObject {
	...
	public void installVpnExit(Expr accessIp, Expr exitIp) {
		...
		constraints.add(ctx.mkForall(new Expr[] { p_0, n_0 }, ctx.mkImplies(
				ctx.mkAnd((BoolExpr) nctx.send.apply(n_VpnExit, n_0, p_0),
						ctx.mkNot((BoolExpr) isInternal.apply(nctx.pf.get("dest").apply(p_0)))),
				ctx.mkExists(new Expr[] { p_1, n_1 },
						ctx.mkAnd(
								(BoolExpr) nctx.recv.apply(n_1, n_VpnExit, p_1), (BoolExpr) isInternal
								...
								ctx.mkEq(nctx.pf.get("inner_dest").apply(p_0), nctx.pf.get("dest").apply(p_1)),
								ctx.mkEq(nctx.pf.get("inner_src").apply(p_0), nctx.pf.get("src").apply(p_1)),
		...
		constraints.add(ctx.mkForall(new Expr[] { p_0, n_0 }, ctx.mkImplies(
				...
				ctx.mkExists(new Expr[] { p_1, n_1 },
						ctx.mkAnd((BoolExpr) nctx.recv.apply(n_1, n_VpnExit, p_1),
