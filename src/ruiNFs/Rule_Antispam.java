...
public class Rule_Antispam extends NetworkObject {
	...
	public void installAntispam() {
		...
		constraints.add(ctx.mkForall(new Expr[] { t_0, p_0, n_0 },
				ctx.mkImplies((BoolExpr) nctx.send.apply(n_Antispam, n_0, p_0, t_0), ctx.mkExists(
						new Expr[] { t_1, p_1, n_1 },
						ctx.mkAnd((BoolExpr) nctx.recv.apply(n_1, n_Antispam, p_1, t_1), ctx.mkLt(t_1, t_0), ctx.mkOr(
								ctx.mkEq(nctx.pf.get("proto").apply(p_1), ctx.mkInt(nctx.POP3_REQUEST)), ctx.mkAnd(
										ctx.mkEq(nctx.pf.get("proto").apply(p_1), ctx.mkInt(nctx.POP3_RESPONSE)),
										ctx.mkNot((BoolExpr) matchEntry.apply(nctx.pf.get("emailFrom").apply(p_1))))),
