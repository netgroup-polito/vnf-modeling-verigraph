	...
	public void installSipServer2(Expr ip_sipServer) {
		...
		constraints
				.add(ctx.mkForall(new Expr[] { p_0, n_0 },
						ctx.mkImplies(ctx.mkAnd((BoolExpr) nctx.send.apply(n_SipServer2, n_0, p_0),
								ctx.mkEq(nctx.pf.get("proto").apply(p_0), ctx.mkInt(nctx.SIP_OK))),
								ctx
										.mkExists(
												new Expr[] { p_1, n_1 },
												ctx.mkAnd((BoolExpr) nctx.recv.apply(n_1, n_SipServer2, p_1),
														ctx.mkEq(nctx.pf.get("proto").apply(p_1),
																ctx.mkInt(nctx.SIP_REGISTER)),
		...
		constraints.add(ctx.mkForall(new Expr[] { p_0, n_0 }, ctx.mkImplies(
				(BoolExpr) nctx.send.apply(n_SipServer2, n_0, p_0),
				ctx.mkExists(new Expr[] { p_1, n_1 }, ctx.mkAnd((BoolExpr) nctx.recv.apply(n_1, n_SipServer2, p_1),
						ctx.mkEq(nctx.pf.get("proto").apply(p_1), ctx.mkInt(nctx.SIP_INVITE)),
						ctx.mkEq(nctx.pf.get("dest").apply(p_1), ip_sipServer),
						ctx.mkExists(new Expr[] { p_2, n_2 },
								ctx.mkAnd((BoolExpr) nctx.recv.apply(n_2, n_SipServer2, p_2),
						...
						ctx.mkEq(nctx.pf.get("encrypted").apply(p_0), nctx.pf.get("encrypted").apply(p_1))), 1, null,
						null, null, null)),
				1, null, null, null, null));
		constraints
				.add(ctx.mkForall(
						...
						ctx.mkImplies(
								(BoolExpr) nctx.send
										.apply(n_SipServer2, n_0,
												p_0),
								ctx.mkExists(new Expr[] { p_1,
										n_1 },
										ctx.mkAnd((BoolExpr) nctx.recv.apply(n_1, n_SipServer2, p_1), ctx.mkOr(
												ctx.mkAnd(ctx.mkEq(nctx.pf.get("proto").apply(p_1),
														ctx.mkInt(nctx.SIP_INVITE)),
														ctx.mkNot(ctx
																.mkEq(nctx.pf.get("dest").apply(p_1), ip_sipServer))),
