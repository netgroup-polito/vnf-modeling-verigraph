...
public class Rule_IPv4Exit extends NetworkObject {
	List<BoolExpr> constraints;
	Context ctx;
	DatatypeExpr n_IPv4Exit;
	Network net;
	NetContext nctx;
	FuncDecl isInternal;
	FuncDecl matchEntry;
	...
	public Rule_IPv4Exit(Context ctx, Object[]... args) {
		super(ctx, args);
	}
	...
	protected void init(Context ctx, Object[]... args) {
		...
		net.saneSend(this);
		isInternal = ctx.mkFuncDecl(n_IPv4Exit + "_isInternal", nctx.address, ctx.mkBoolSort());
		matchEntry = ctx.mkFuncDecl(n_IPv4Exit + "_matchEntry", new Sort[] { nctx.address }, ctx.mkBoolSort());
	...
	protected void addConstraints(Solver solver) {
		...
		if (entries.size() == 0)
			return;
		Expr e_0 = ctx.mkConst(n_IPv4Exit + "_entry_e_0", nctx.address);
	...
	public void setInternalAddress(ArrayList<DatatypeExpr> internalAddress) {
		...
		Expr in_0 = ctx.mkConst(n_IPv4Exit + "_internal_node", nctx.address);
	...
	public void installIPv4Exit(Expr accessIp, Expr exitIp) {
		Expr n_0 = ctx.mkConst("n_IPv4Exit_" + n_IPv4Exit + "_n_0", nctx.node);
		Expr n_1 = ctx.mkConst("n_IPv4Exit_" + n_IPv4Exit + "_n_1", nctx.node);
		Expr p_0 = ctx.mkConst("n_IPv4Exit_" + n_IPv4Exit + "_p_0", nctx.packet);
		Expr p_1 = ctx.mkConst("n_IPv4Exit_" + n_IPv4Exit + "_p_1", nctx.packet);
		constraints
				.add(ctx.mkForall(
						new Expr[] { p_0, n_0 }, ctx
								.mkImplies(
										ctx.mkAnd((BoolExpr) nctx.send.apply(n_IPv4Exit, n_0, p_0),
												ctx.mkNot(
														ctx.mkEq(nctx.pf.get("inner_src").apply(p_0),
																nctx.am.get("null")))),
										ctx.mkExists(new Expr[] { p_1, n_1 }, ctx.mkAnd(
												(BoolExpr) nctx.recv.apply(n_1, n_IPv4Exit, p_1),
												...
												ctx.mkEq(nctx.pf.get("dest").apply(p_0), accessIp),
												ctx.mkEq(nctx.pf.get("src").apply(p_0), exitIp),
												...
												(BoolExpr) isInternal.apply(nctx.pf.get("inner_dest").apply(p_0)),
		...
		constraints.add(ctx.mkForall(new Expr[] { p_0, n_0 }, ctx.mkImplies(
				...
				ctx.mkExists(new Expr[] { p_1, n_1 },
						ctx.mkAnd((BoolExpr) nctx.recv.apply(n_1, n_IPv4Exit, p_1),
								ctx.mkEq(nctx.pf.get("src").apply(p_1), accessIp),
								ctx.mkEq(nctx.pf.get("dest").apply(p_1), exitIp),
								ctx.mkNot(ctx.mkEq(nctx.pf.get("inner_src").apply(p_1), nctx.am.get("null"))),
								...
								(BoolExpr) isInternal.apply(nctx.pf.get("src").apply(p_0)),
