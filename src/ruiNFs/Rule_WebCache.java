...
public class Rule_WebCache extends NetworkObject {
	...
	public void installWebCache() {
		...
		constraints.add(ctx.mkForall(new Expr[] { p_0, n_0 },
				ctx.mkImplies(
						ctx.mkAnd((BoolExpr) nctx.send.apply(n_WebCache, n_0, p_0),
								(BoolExpr) isInternal.apply(nctx.pf.get("dest").apply(p_0))),
						ctx.mkExists(new Expr[] { p_1, n_1 }, ctx.mkAnd(
								(BoolExpr) nctx.recv.apply(n_1, n_WebCache, p_1),
								...
								ctx.mkExists(new Expr[] { p_2, n_2 },
										ctx.mkAnd((BoolExpr) nctx.recv.apply(n_2, n_WebCache, p_2),
												...
												ctx.mkEq(nctx.pf.get("url").apply(p_0), nctx.pf.get("url").apply(p_2))),
				...
				1, null, null, null, null));
		constraints.add(ctx.mkForall(new Expr[] { p_0, n_0 },
				ctx.mkImplies(ctx.mkAnd((BoolExpr) nctx.send.apply(n_WebCache, n_0, p_0),
						ctx.mkNot((BoolExpr) isInternal.apply(nctx.pf.get("dest").apply(p_0)))),
						ctx
								.mkExists(
										new Expr[] { p_1,
												n_1 },
										ctx.mkAnd((BoolExpr) nctx.recv.apply(n_1, n_WebCache, p_1),
												(BoolExpr) isInternal.apply(nctx.pf.get("src").apply(p_1)),
												ctx.mkEq(nctx.pf.get("proto").apply(p_1), ctx.mkInt(nctx.HTTP_REQUEST)),
