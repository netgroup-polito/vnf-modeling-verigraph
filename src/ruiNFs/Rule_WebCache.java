	...
	public void installWebCache() {
		...
		constraints.add(ctx.mkForall(
				new Expr[] { t_0, p_0, n_0 }, ctx
						.mkImplies(
								ctx.mkAnd((BoolExpr) nctx.send.apply(n_WebCache, n_0, p_0, t_0),
										(BoolExpr) isInternal.apply(nctx.pf.get("dest").apply(p_0))),
								ctx.mkExists(
										new Expr[] { t_1, p_1, n_1 }, ctx
												.mkAnd((BoolExpr) nctx.recv.apply(n_1, n_WebCache, p_1, t_1),
														ctx.mkLt(t_1, t_0),
														...
														ctx.mkExists(new Expr[] { t_2, p_2, n_2 }, ctx.mkAnd(
																(BoolExpr) nctx.recv.apply(n_2, n_WebCache, p_2, t_2),
																ctx.mkLt(t_2, t_1),
		...
		constraints.add(ctx.mkForall(new Expr[] { t_0, p_0, n_0 }, ctx.mkImplies(
				...
				ctx.mkExists(new Expr[] { t_1, p_1, n_1 },
						ctx.mkAnd((BoolExpr) nctx.recv.apply(n_1, n_WebCache, p_1, t_1), ctx.mkLt(t_1, t_0),
								(BoolExpr) isInternal.apply(nctx.pf.get("src").apply(p_1)),
