...
public class Rule_SipServer extends NetworkObject {
	...
	public void installSipServer(Expr ip_sipServer, Expr domain, Expr ip_dns) {
		...
		constraints.add(ctx.mkForall(new Expr[] { p_0, n_0 },
				ctx.mkImplies((BoolExpr) nctx.send.apply(n_SipServer, n_0, p_0), ctx.mkExists(new Expr[] { p_1, n_1 },
						ctx.mkAnd((BoolExpr) nctx.recv.apply(n_1, n_SipServer, p_1),
								ctx.mkEq(nctx.pf.get("proto").apply(p_1), ctx.mkInt(nctx.SIP_REGISTE)),
								...
								ctx.mkEq(nctx.pf.get("proto").apply(p_0), ctx.mkInt(nctx.SIP_REGISTE_OK)),
