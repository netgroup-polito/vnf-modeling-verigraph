...
public class Rule_GlobalDnsBalancer extends NetworkObject {
	...
	protected void init(Context ctx, Object[]... args) {
		...
		matchEntry = ctx.mkFuncDecl(n_GlobalDnsBalancer + "_matchEntry", new Sort[] { nctx.address, ctx.mkIntSort() },
				ctx.mkBoolSort());
	...
	public void installGlobalDnsBalancer(Expr ip_url) {
		...
		constraints.add(ctx.mkForall(new Expr[] { p_0, n_0 }, ctx.mkImplies(
				(BoolExpr) nctx.send.apply(n_GlobalDnsBalancer, n_0, p_0),
				ctx.mkExists(new Expr[] { p_1, n_1 },
						ctx.mkAnd((BoolExpr) nctx.recv.apply(n_1, n_GlobalDnsBalancer, p_1),
								...
								ctx.mkEq(nctx.pf.get("inner_dest").apply(p_0), ip_url),
								...
								ctx.mkEq(nctx.pf.get("proto").apply(p_0), ctx.mkInt(nctx.DNS_RESPONSE)),
