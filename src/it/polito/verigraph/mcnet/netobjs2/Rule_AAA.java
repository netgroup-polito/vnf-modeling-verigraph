		...
		constraints
				.add(ctx.mkForall(
						new Expr[] { p_0, n_0 }, ctx
								.mkImplies(
										(BoolExpr) nctx.send.apply(n_AAA, n_0, p_0), ctx
												.mkExists(
														new Expr[] { p_1, n_1 }, ctx.mkAnd(
																(BoolExpr) nctx.recv.apply(n_1, n_AAA, p_1),
																ctx.mkEq(
																		nctx.dest_port.apply(p_1),
																		ACCOUNTING_PORT_1813),
																(BoolExpr) matchEntry.apply(
																		nctx.pf.get("body").apply(p_1)),
																ctx.mkEq(nctx.pf.get("src").apply(p_0),
																		nctx.pf.get("dest").apply(p_1)),
																ctx.mkEq(nctx.src_port.apply(p_0),
																		nctx.dest_port.apply(p_1)),
																ctx.mkEq(nctx.pf.get("dest").apply(p_0),
																		nctx.pf.get("src").apply(p_1)),
																ctx.mkEq(nctx.dest_port.apply(p_0),
																		nctx.src_port.apply(p_1)),
