...
public class StatementVisitor extends ASTVisitor {
	...
	public boolean visit(IfStatement node) {
		...
		if (!visitorIf.hasReturnStatement() && !visitorElse.hasReturnStatement()) // No return stmt inside this
																					// IF-THEN-ELSE
			return false;
	...
	public boolean visit(Assignment node) {
		...
		System.out.println(
				"\t\tType -> " + ASTNode.nodeClassForType(node.getRightHandSide().getNodeType()).getSimpleName());
		List<Variable> var = variables.get(simpleName);

		if (var != null && !isGlobal) {
			Variable v = new Variable(simpleName, node.getRightHandSide(), var.get(0).getType(),
					var.get(0).getTypeName());
			var.add(v);
	...
	public boolean visit(ReturnStatement node) {
		...
		System.out.println("\t*****Found action " + r.getAction() + " (with " + conditions.size()
				+ " conditions leading to it) at line " + compilationUnit.getLineNumber(node.getStartPosition())
				+ ". Reached nesting level " + nestingLevel);

		int j = 1;
		for (IfElseBranch ieb : previousIfElseBranch)
			System.out.println("\t\tPrevious IF-THEN-ELSE #" + j++ + " " + ieb.getStatement().getExpression()
					+ " (IF has RETURN=" + ieb.ifBranchContainsReturn() + ") (ELSE has RETURN="
					+ ieb.elseBranchContainsReturn() + ")");

		j = 1;
		for (IfElseBranch ieb : conditions)
			System.out.println(
					"\t\tCondition #" + j++ + " (" + ieb.getBranch() + "): " + ieb.getStatement().getExpression());

		for (ForLoop fLoop : loops)
			System.out.println("\tEncountered " + fLoop.getStatement().getExpression() + " with nesting level "
					+ fLoop.getNestingLevel());
		...
		if (r.getAction() == Action.FORWARD) {
			...
			if (!statementContext.getMethodContext().getMethodName().equals(Constants.DEFINE_SENDING_PACKET_METHOD))
				statementContext.getMethodContext().addReturnSnapshots(
						statementContext.createSnapshot(r.getPacketName(), r.getInterfaceName(), false));
			else {
				statementContext.getMethodContext().addReturnSnapshots(
						statementContext.createSnapshot(r.getPacketName(), r.getInterfaceName(), true));
	...
	public boolean visit(EnhancedForStatement node) {
		System.out.println("\tFound FOR-EACH. Variable declaration -> " + node.getParameter() + ". Iterate over -> "
				+ node.getExpression());
		...
		// new Statement2Visitor(compilationUnit, variables, c, false,
		// previousIfElseBranch, loops, nestingLevel+1, predicatesOnSentPacket,
		// skippedActions);
			...
			if (fragment.getInitializer() != null) {
				fragment.getInitializer().accept(new ASTVisitor() {
					...
					public boolean visit(MethodInvocation node) {
						 ...
						 * for(Expression exp : (List<Expression>) node.arguments()){
						 * 
						 * exp.accept(new ASTVisitor() { public boolean visit(QualifiedName node){
						 * StringBuilder builder = new StringBuilder();
						 * builder.append(node.getName().getFullyQualifiedName());
						 * System.out.println("//////////getField="+builder.toString()); return false; }
						 * });
						 * 
						 * //v.addMatchedField(exp); }
						 * 
