...
/**
 * Main class performing the Stage 1 parsing phase. Main tasks of this phase: -
 * scan the variables in the source code; - find matchEntry() method calls and
 * parse the involved fields (which fields are being matched) - find
 * storeEntry() method calls and parse the involved fields (which fields are
...
public class ClassVisitor extends ASTVisitor {
	...
	/*
	 * A map containing each encountered identifier and the corresponding
	 * information (Variable class)
	 */
	private Map<String, Variable> variables;

	/*
	 * The name of the variable used for the VNF's table (FIXME: can be statically
	 * choosen?)
	 */
	...
	/**
	 * This is called every time a member variable declaration is found inside the
	 * class source code (i.e. all the variables declared inside the class but
	 * outside any method).
	 *
	 * @param node
	 *            the FieldDeclaration statement
	...
	public boolean visit(FieldDeclaration node) {
		...
		/*
		 * We want to find annotations on the variables (done by AnnotationVisitor
		 * class).
		 */
		...
		/*
		 * We have a list because a declaration statement can contain multiple decls
		 * separated by commas
		 */
		for (VariableDeclarationFragment fragment : (List<VariableDeclarationFragment>) fragments) {
			...
			if (aVisitor.foundAnnotation()) {
				/*
				 * We have two types of class member we are interested in: CONF member (like
	 ...
	 *
	 * @param node
	 *            the MethodDeclaration statement
	 * @return whether to continue the parsing after this MethodDeclaration
	 *         statement
	 */
	@Override
	public boolean visit(MethodDeclaration node) {
		...
		// System.err.println("[ERROR] The "+ Constants.MAIN_NF_METHOD + "() method MUST
		...
		for (SingleVariableDeclaration singleVariableDeclaration : l) {
			...
			Variable v = new Variable(singleVariableDeclaration.getName().getFullyQualifiedName(),
					singleVariableDeclaration.getInitializer(), Type.GENERIC, variableTypeName);
