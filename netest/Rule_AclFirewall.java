...
class SymRule_AclFirewall {
  ...
  public InstructionBlock[] addrule(  String[][] p){
    ...
    for (int i=0; i < p.length(); i++) {
      rule=Array(InstructionBlock(If(Constrain(Tag("IP_SRC"),ConstantValue(ipToNumber(p(i + 0)))),InstructionBlock(If(Constrain(Tag("IP_DST"),ConstantValue(ipToNumber(p(i + 1)))),Fail("Match"),NoOp)),NoOp)));
