...
public class Rule_Ids extends NetworkObject {
	...
	public void installIds() {
		...
		constraints.add(ctx.mkForall(new Expr[] { t_0, p_0, n_0 }, ctx.mkImplies(
				ctx.mkAnd((BoolExpr) nctx.send.apply(n_Ids, n_0, p_0, t_0),
						ctx.mkEq(nctx.pf.get("proto").apply(p_1), ctx.mkInt(nctx.HTTP_REQUEST))),
				ctx.mkExists(new Expr[] { t_1, p_1, n_1 },
						ctx.mkAnd((BoolExpr) nctx.recv.apply(n_1, n_Ids, p_1, t_1), ctx.mkLt(t_1, t_0),
								ctx.mkEq(nctx.pf.get("proto").apply(p_1), ctx.mkInt(nctx.HTTP_REQUEST)),
						...
						new Expr[] { t_0, p_0, n_0 }, ctx
								.mkImplies(
										...
										ctx.mkExists(new Expr[] { t_1, p_1, n_1 }, ctx.mkAnd(
												(BoolExpr) nctx.recv.apply(n_1, n_Ids, p_1, t_1),
												ctx.mkLt(t_1, t_0),
												ctx.mkNot(
